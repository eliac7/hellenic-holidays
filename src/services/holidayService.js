...
const holidays_1 = require("../data/holidays");
class HolidayService {
    ...
    getHolidays(year) {
        ...
        const fixedDates = holidays_1.fixedHolidays.map((holiday) => (Object.assign(Object.assign({}, holiday), { date: holiday.getDate(year) })));
        // Get movable holidays
        const movableDates = (0, holidays_1.getMovableHolidays)(year);
        // Combine and sort all holidays by date
        return [...fixedDates, ...movableDates].sort((a, b) => a.date.getTime() - b.date.getTime());
    }
    /**
     ...
     * @returns Array of holidays
     */
    getHolidaysBetweenDates(startDate, endDate) {
        ...
        return holidays
            .filter((holiday) => (this.isSameDay(holiday.date, startDate) ||
    ...
    getNextHoliday(date) {
        ...
        return holidays.find((holiday) => holiday.date.getTime() > date.getTime() ||
